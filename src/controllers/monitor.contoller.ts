import { Controller, Example, Get, Request, Response, Route, Security, Tags } from "tsoa";
...
export class MonitorController extends Controller {
    ...
    @Response<HealthResponse>(500, "INTERNAL_SERVER_ERROR", {
        status: "fail",
...
export const healthCheck = async (req, res, next) => {
    try {
        ...
        res.status(await controller.getStatus()).json(result);
    } catch (e) {
        logger.error(e);
...
export const diskCheck = async (req, res, next) => {
    try {
        const controller = new MonitorController();
        const result = await controller.getDisk();
...
export const authCheck = async (req, res, next) => {
    try {
        ...
        res.status(await controller.getStatus() || httpStatus.OK).json(result);
    } catch (e) {
        logger.error(e);
        const statusCode = e.statusCode || httpStatus.INTERNAL_SERVER_ERROR;
        errorHandler(new ApiError(statusCode, null, e.message, { stack: e.stack, isFatal: true }), req, res, next);
